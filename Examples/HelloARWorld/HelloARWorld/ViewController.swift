...
//  HelloARWorld
//
//  Created by Jackson on 4/9/19.
//  Copyright © 2019 Jackson. All rights reserved.
//
// Put scenekit objects into the real world...and we do some orientation too
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    func drawSphereAtOrigin() {
        // need to specify shape, diffuse/specular, and position, and add it to the root node
    ...
    func drawOrbitingShip() {
        let scene = SCNScene(named: "art.scnassets/ship.scn")! // get the asset
        let ship = (scene.rootNode.childNode(withName: "ship", recursively: false))!// extract the asset
        ship.position = SCNVector3(1, 0, 0)
        ship.eulerAngles = SCNVector3(0, 180.degreesToRadians(), 0)
        ship.scale = SCNVector3(0.3, 0.3, 0.3) // scale it down
        sphere.addChildNode(ship) // because it is orbiting the sphere
    }
    ...
    func drawTorusAt300() {
        ...
        let firstMat = torus.geometry?.firstMaterial
        firstMat!.diffuse.contents = UIColor.red
        firstMat!.specular.contents = UIColor.white
